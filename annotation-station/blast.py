...
def is_positive_rna_count(chrom, pos, blast_result_dicts,
    ...
    blast_result_dicts = sorted(blast_result_dicts, key=lambda x: x['bitscore'], reverse=True)
    # throw out less than 90% coverage
    blast_result_dicts = [d for d in blast_result_dicts
            if int(d['qend']) - int(d['qstart']) / d['qlen'] >= coverage_threshold]
...
def BlastAnnotator(object):
    ...
    def __init__(self, annotations, database='GRCh38.d1.vd1.fa', max_target_seqs=5, max_hsps=5,
        ...
        """
        Possible annotations - rna_editing, avg_top_hits
        """
    ...
    def get_rna_editing_annotations(self, position_to_read_results):
        ...
        for (chrom, pos), read_to_result_dicts in position_to_read_results.items():
            ...
            count = 0
            for result_dict in read_to_result_dicts.values():
                if is_positive_rna_count(chrom, pos, blast_result_dicts,
    ...
    def get_blast_annotations(self, input_fasta, chrom_regex=r'^(.*):.*$',
        ...
        position_to_read_results = defaultdict(defaultdict(list))
        for sequence_id, result_dict in sequence_to_results.items():
            ...
            pos_tup = (chrom, pos)
            position_to_read_results[pos_tup][read].append(result_dict)
